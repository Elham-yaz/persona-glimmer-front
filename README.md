...
- **`post-6` wording.** "The agent resolved my issue to my satisfaction" presupposes a service problem, which the informational context does not have. Whether it stays, is reworded, or becomes context-conditional is an open decision; item text lives in both `backend/src/seeds/survey_questions.seed.ts` and `src/data/surveyQuestions.ts` and must be changed in both.
