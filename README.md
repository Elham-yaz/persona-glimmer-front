...
A web platform for a behavioral study of how a customer-service AI agent's **emotional intelligence (EI) × cognitive intelligence (CI)** profile shapes the way people experience a short service conversation. Each participant arrives from a Qualtrics survey, is randomly assigned to one of **12 cells** — 4 agent conditions (EI high/low × CI high/low, all presented identically as an unnamed "AI agent") × 3 conversation contexts (a *utilitarian* food-delivery failure, a *hedonic* food-delivery failure, and an *informational* food-delivery inquiry with no service issue) — has exactly **10 interactions** with the agent (one participant message plus one agent reply each; the scenario text is auto-sent as interaction 1), answers a **16-item post-chat questionnaire**, receives a **unique 5-digit completion code**, and returns to Qualtrics. No accounts, no login, no personal data: a session is an anonymous UUID held in the participant's browser. Researchers monitor cell balance and export transcripts and survey responses from a key-gated admin dashboard.
...
│   │   ├── seeds/                 agent_conditions, contexts (context 3 = PLACEHOLDER), survey_questions, guardrails
...
`MAX_INTERACTIONS = 10`, `PROMPT_VERSION = "2.2"` (`"2.1"` before 2026-09-14), 16 survey items (`post-1 … post-16`) with values 1–7, completion codes in `10000–99999` with up to 20 collision retries. The frontend reads `maxInteractions` from API responses rather than hardcoding it; the 16 survey items (ids, text and the instruction sentence; survey version `2.0`) are duplicated in `src/data/surveyQuestions.ts` and must be kept in sync with the backend seed, and the admin grid hardcodes the id ranges 1–4 / 1–3 for layout.
...
`db:reset` prints the host and database it is about to wipe and refuses unless `CONFIRM_RESET` matches the database name in `DATABASE_URL` exactly. After any later push that adds a migration or changes a seed, run `npm run migrate` and/or `npm run seed` again the same way. A deploy and a re-seed are not atomic: `prompt_version` is stamped from code (`PROMPT_VERSION` in `backend/src/config/study.ts`), but the agent's display name, the shared prompt template and the survey item text come from the seeded tables, which nothing updates at deploy time. For the 2026-09-14 amendment (`PROMPT_VERSION = "2.2"`, unnamed agent, new instrument) pause the Qualtrics link, deploy the backend and the frontend, run `npm run seed` on the production database, smoke-test, and only then resume: a session created after the backend deploy but before the re-seed is stamped `2.2` yet chatted with the old named agent and template (the reverse order stamps `2.1` on sessions that saw the new ones), so exclude any session whose `created_at` falls in that gap. Without Shell access, run the same commands locally with `DATABASE_URL` set to the database's **External** URL (`?sslmode=require`).
...
- **What is stored** (PostgreSQL, see the schema in [docs/STUDY2_API.md](./docs/STUDY2_API.md) §4): one `sessions` row per participant (assignment, `external_id`, interaction count, lock/complete flags and timestamps, completion code, `model`, `prompt_version`); every message in `session_messages` (`sequence` 1–20, `role`, `content`, `client_message_id`); every answer in `session_survey_responses` (`question_id`, `response_value`); plus reference tables `agent_conditions`, `contexts`, `survey_questions` (the frozen item text, version `2.0` since 2026-09-14) and `global_guardrails`.
- **What is not stored:** names, emails, IP addresses, user agents, device information, or anything typed outside the chat and the questionnaire. There are no user accounts. Participants are only ever shown the agent as "AI agent" (it has no name) and the scenario — never the EI/CI levels or the condition code.
...
- **Reading the exports.** Use `agent_code` (`hiEI_hiCI`, `hiEI_loCI`, `loEI_hiCI`, `loEI_loCI`) and `context_code` (`food_utilitarian`, `food_hedonic`, `food_informational`) as factors. The messages export is ordered by session start and then `sequence`, so each session's transcript is contiguous and in order (odd sequences are the participant, even the agent; sequence 1 is the auto-sent scenario). `is_fallback` is always `false` in Study 2 — a failed model call persists nothing — and is kept only for schema compatibility. `model` and `prompt_version` (`2.2` since 2026-09-14; `2.1` before, when the agent carried a human first name and the survey items were the Study-1 set — split on `prompt_version` before pooling survey responses) are stamped per session for reproducibility; sessions created with `MOCK_OPENAI=true` show `model = "mock"` and should be excluded.
...
- **Control context is placeholder content.** Context 3 (`food_informational`, "Delivery Order Inquiry") — its reference document (order record, ingredient and allergen lists, packaging and delivery-process information) and participant scenario — was fabricated for development and pilot testing. The real material from the research team goes into `backend/src/seeds/contexts.seed.ts` (one edit), followed by `npm run seed`. Keep any "placeholder" wording out of the seeded text itself: it is pasted verbatim into the agent's prompt.
- **Instrument wording in the informational context.** The post-chat instrument supplied by the researchers on 2026-09-14 (survey version `2.0`) is asked verbatim in all three contexts. Its instruction sentence ("… during the service recovery") and several items (`post-1`, `post-2`, `post-5`, `post-10`, `post-11`) refer to a service problem, which the informational context does not have. Any change is the research team's call; item text lives in both `backend/src/seeds/survey_questions.seed.ts` and `src/data/surveyQuestions.ts` and must be changed in both.
